# VBI-modal-analysis
MATLAB code for "Improved vehicle-bridge interaction element including speed effects for time-varying modal analysis".

Overview
This MATLAB program computes the dynamic response and coupled eigenvalues of a single-degree-of-freedom sprung mass moving on a simply supported Bernoulli-Euler beam. The primary objective is to demonstrate how the configuration definition parameter γ affects the system eigenvalues (coupled frequencies and damping ratios).
...
f00_mainProgram.m Main script — run this file to execute the full analysisf01_loadInputParams.mDefines bridge, vehicle, and solver parameters (including γ)
f02_buildElementMatrices.m Builds beam element matrices (M, K, C) and computes uncoupled frequencies
...
f05_solveNewmarkBeta.m Solves the dynamic response using the Newmark-β method (constant average acceleration)
