...
% =========================================================================
% Description : Solve the dynamic response of the multi-vehicle VBI system
%               using the Newmark-beta method (beta = 1/4, gamma_NM = 1/2).
% Author      : Shi LuoKe (史罗克)
% Date        : 2026-04-18
% -------------------------------------------------------------------------
% Input  : globalMat  - Global matrices structure
%          sysParams  - System parameter structure
% Output : dynResult  - Dynamic response (bridge midspan only)
% =========================================================================
...
function dynResult = f05_solveNewmarkBeta(globalMat, sysParams)
    ...
    Dof        = globalMat.Dof;
    Dof_bridge = globalMat.Dof_bridge;
    Num_steps  = globalMat.Num_steps;

    %% Initialize displacement, velocity, and acceleration arrays
    d = zeros(Dof, Num_steps);
    v = zeros(Dof, Num_steps);
    a = zeros(Dof, Num_steps);
    ...
    %% Extract bridge mid-span response
    if mod(n,2) ~= 0
